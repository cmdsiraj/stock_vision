...
def get_current_day_stocks():
    ticker_list=['AAPL','GOOGL','TSLA',"AMZN"]
    date=datetime.today()
    previous_date = date - timedelta(days=1)
    previous_date1=str(previous_date.month)+"/"+str(previous_date.day)+"/"+str(previous_date.year)
    days_before_date= date - timedelta(days=5)
    days_before_date1=str(days_before_date.month)+"/"+str(days_before_date.day)+"/"+str(days_before_date.year)
    data=list()
    ...
    for i in ticker_list:
        stock_data=get_data(i,start_date=days_before_date1,end_date=previous_date1)
